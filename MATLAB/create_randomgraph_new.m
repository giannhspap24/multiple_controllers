function [G,Gtest,x_coords,y_coords] = create_randomgraph_new(num_nodes)
    ...
    while (connected==0)
        ...
        for i=1:num_nodes
            ...
            x_coords(i) = 0 + (40-0).*rand(1,1);
            y_coords(i) = 0 + (40-0).*rand(1,1);
        end
        ...
        for i=1:size(G.Edges,1)
            ...
            if(distances(i)<4) %4,6
                G.Edges{i,2}=2;
                %edgesremove(end+1)=i;
            end
            if(distances(i)<2.9) %4.73, 2.9
                G.Edges{i,2}=3;
                %edgesremove(end+1)=i;
            end
        end
        ...
        if (max(components) == 1)&&(numedges( G )<71) &&(numedges( G )>40)
