function [G,Gtest] = create_random_graph_RGG_neww(num_nodes)
    ...
    while (connected==0)
        ...
        for i=1:num_nodes
            ...
            x_coords(i) = 0 + (A-0).*rand(1,1);
            y_coords(i) = 0 + (A-0).*rand(1,1);
        end
        ...
        for i=1:size(G.Edges,1)
            ...
            if(distances(i)<(A/6)) %4,6
                G.Edges{i,2}=2;
                %edgesremove(end+1)=i;
            end
            if(distances(i)<(A/8)) %4.73, 2.9
                G.Edges{i,2}=3;
                %edgesremove(end+1)=i;
            end
        end
        ...
        if (max(components) == 1)&&(numedges( G )<(num_nodes*1.5)) &&(numedges( G )>(num_nodes))
